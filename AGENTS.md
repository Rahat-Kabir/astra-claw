...
- Preview-and-approve: `write_file`/`patch` build a unified diff and call `path_safety.request_write_approval(path, diff, action)` before writing. The callback is module-level (`set_write_approval_callback`), mirroring `shell_tool`; no callback registered = allow (one-shot/tests/scripts never hang). Rejection returns `{"status": "rejected_by_user"}` so the model re-plans. The REPL registers `_build_write_approval_callback` (y/n/a with a session-wide "always" latch) when `cli.confirm_edits` is true and clears it on exit. `unified_diff()` lives in `path_safety.py` and is shared by both tools.
